...
/**
 * Streaming proxy to `GET /conversations/{id}/events` (SSE). Unlike
 * `proxyBackendJson`, this never awaits or re-serializes the backend body --
 * doing so would buffer the entire (deliberately infinite) stream before
 * anything reaches the browser. The backend response's `body` is a
 * `ReadableStream`; it's returned as-is.
 *
...
export async function GET(
  ...
  try {
    backendResponse = await fetch(`${BACKEND_API_URL}/conversations/${conversationId}/events`, {
      headers,
    });
  } catch {
