...
export function PanelTeam({
  ...
  const recordTiles = [
    {
      label: "Won",
      value: String(record.won),
      className: "text-data-positive",
    },
    {
      label: "Drawn",
      value: String(record.drawn),
      className: "text-data-neutral",
    },
    {
      label: "Lost",
      value: String(record.lost),
      className: "text-data-negative",
    },
    {
      label: "Goals for",
      value: String(record.goalsFor),
      className: "text-ink-primary",
    },
    {
      label: "Against",
      value: String(record.goalsAgainst),
      className: "text-ink-primary",
    },
    {
      label: "Goal diff",
      ...
      className:
        team.goalDifference >= 0 ? "text-data-positive" : "text-data-negative",
    },
  ];
    ...
    >
      ...
      <section className="flex shrink-0 flex-col gap-3 border-b border-border-subtle p-5">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span className="text-label-sm text-ink-muted">
            {team.matchResults.length} played
          </span>
        </div>
        <div className="flex flex-col">
          {team.matchResults.map((row) => (
            ...
            >
              <span className="w-8 text-label-sm text-ink-muted">
                {row.stage}
              </span>
              <AvatarBadge
                label={row.opponentCode}
                size={28}
                className="text-[9px]"
              />
      ...
      <section className="flex grow flex-col gap-3 p-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          <StatChip
            variant="tile"
            label="Yellow cards"
            value={String(team.discipline.yellowCards)}
          />
          <StatChip
            variant="tile"
            label="Red cards"
            value={String(team.discipline.redCards)}
          />
          <StatChip
            variant="tile"
            label="Fouls"
            value={String(team.discipline.fouls)}
          />
        </div>
        <span className="text-body-sm text-ink-muted">
          {team.discipline.yellowPerMatch.toFixed(2)} yellow cards per match ·
          field {team.discipline.fieldYellowPerMatch.toFixed(2)}
        </span>
      </section>
    </aside>
  );
